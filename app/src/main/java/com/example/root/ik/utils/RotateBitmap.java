...
public class RotateBitmap {
    ...
    private static Bitmap rotateImage(Bitmap image, int degree){
        //TODO: read about the matrix class
        Matrix matrix = new Matrix();
        matrix.postRotate(degree);
        Bitmap rotateImg = Bitmap.createBitmap(image,0,0,image.getWidth(),image.getHeight(),matrix,true);
        image.recycle();
        return rotateImg;
    }
    /**
     ...
     * @throws IOException
     */
    public Bitmap HandleSamplingAndRotationBimap(Context context, Uri selectedImage) throws
            IOException{
        mContext = context;
        int MAX_HEIGHT = 1024;
        int MAX_WIDTH = 1024;
        ...
        image = rotateImageIfRequired(image,selectedImage);
        return image;
    }
